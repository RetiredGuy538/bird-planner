...
def build_html(weather_json, haikubox_data=None):
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .pick-date{font-size:11px;color:var(--sub);margin:4px 0 14px;}
    ...
    .sp-row{display:flex;align-items:center;padding:6px 0;
      border-bottom:1px solid rgba(255,255,255,.04);font-size:12px;}
    .sp-row:last-child{border-bottom:none;}
    .sp-name{flex:1;color:var(--text);}
    .sp-cnt{color:var(--sub);font-size:11px;margin-left:8px;}
    .sp-ago{color:var(--gold);font-size:10px;margin-left:8px;white-space:nowrap;}
    ...
    def hb_panel():
        ...
        NOTABLE = {'Bald Eagle','Sandhill Crane','Great Horned Owl','Merlin',
                   'Red-headed Woodpecker','Virginia Rail','Golden-crowned Kinglet',
                   'Common Loon','Osprey','Short-eared Owl','Peregrine Falcon'}
        rows = ''
        for sp in species_list[:16]:
            star = ' ⭐' if sp['name'] in NOTABLE else ''
            rows += f"""<div class="sp-row">
              <div class="sp-name">{sp['name']}{star}</div>
        ...
        return f"""<div class="hb-grid">
          <div class="hb-card">
            <h3>🎙 Recently on your deck <span class="hb-meta">last 24h · {total} detections</span></h3>
            {rows}
            <div style="margin-top:12px;font-size:10px;color:var(--sub)">
              ⭐ Notable species &nbsp;·&nbsp;
              <a href="https://birds.haikubox.com/listen/{HAIKUBOX_ID}" target="_blank"
                 style="color:var(--gold);text-decoration:none">Listen live →</a>
            </div>
          </div>
          <div class="hb-card">
            <h3>📊 Peak activity <span class="hb-meta">peak: {fmt12(peak_hr)}</span></h3>
            <div class="act-chart">{bars}</div>
            <div style="margin-top:10px;font-size:10px;color:var(--sub);line-height:1.7">
              {len(species_list)} species · {total} detections in last 24 hrs<br>
              Haikubox "Lake Koshkonong" · {HAIKUBOX_ID[:6]}…
            </div>
          </div>
        </div>"""
